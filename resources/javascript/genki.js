  ...
  var Genki = {
    ...
    lang : {
      ...
      opts : {
        ...
        stroke : {
          stroke : 'Stroke Order Practice',
          drawing : 'Extended Drawing Practice'
        }
      },
    ...
    generateQuiz : function (o) {
      ...
      *********************************/
      ...
      if (typeof o.type === 'object') {
        ...
        // example: https://sethclydesdale.github.io/tobira-study-resources/lessons/lesson-1/vocab-1/?begin=0 (this starts multiple choice, 1 would start drag and drop)
      ...
      if (o.type == 'drag') {
        ...
        if (storageOK && (localStorage.vocabHorizontal == 'true' || !localStorage.vocabHorizontal)) {
        ...
        zone.innerHTML = '<div id="quiz-info">' + o.info + '<br><span class="en">If you don\'t know how to type in Japanese on your computer, please visit our help page by <a href="../../../help/writing/' + Genki.local + '" target="_blank">clicking here</a>.</span><span class="ja">パソコンで日本語を入力する方法がわからない場合は、<a href="../../../help/writing/' + Genki.local + '" target="_blank">ヘルプページ</a>をご覧ください。</span></div><div class="text-block">' + o.quizlet.replace(/\{.*?\}/g, function (match) {
          ...
          if (data[0] == '!IMG') {
            ...
            return '<span class="writing-zone">'+
              ...
              ).replace(/(\!I.*?$)/, function (M, $1) { // text insert triggers (for particle/word selection exercises)
                if (/^!I/.test($1)) {
                  for (var trigger = $1.slice(2).split('／'), i = 0, j = trigger.length, str = ''; i < j; i++) {
                    str += '<a href="#" onclick="this.parentNode.parentNode.querySelector(\'.writing-zone-input\').value = this.innerText; return false;" class="insert-trigger">' + trigger[i] + '</a>／';
                  }
          ...
          quiz += '<div id="quiz-q' + i + '" class="question-block" data-qid="' + (i + 1) + '" style="display:none;">'+
            ...
            '<div class="quiz-multi-question">'+
              ...
              '<div class="kanji-stroke-order">'+
                '<a class="button" href="' + (q[i].kana ? getPaths() + 'resources/images/stroke-order/sasagami-' + o.kanaType.toLowerCase() + '.jpg' : 'https://jisho.org/search/' + q[i].kanji + '%20%23kanji') + '" target="_blank" title="' + (GenkiLang == 'ja' ? 'jisho.orgで書き順を見る' : 'View stroke order on jisho.org') + '"><i class="fa">&#xf002;</i><span class="en">View on jisho.org</span><span class="ja">jisho.orgで見る</span></a><br>'+
                '<div class="stroke-order-container"><img src="' + img + '" alt="' + ( GenkiLang == 'ja' ? '書き順' : 'stroke order' ) + '"/></div>'+
              '</div>'+
            '</div>'+
            ...
            '</div>'+
            '<div class="quiz-multi-row">'+
              '<div tabindex="0" class="quiz-multi-answer next-question" onclick="Genki.progressQuiz(this, false, \'stroke\');" onkeypress="event.key == \'Enter\' && Genki.progressQuiz(this, false, \'stroke\');" data-canvas="canvas-' + i + '"><span class="en">Next '+ (q[i].kana ? o.kanaType : 'Kanji') + '</span><span class="ja">次の' + (q[i].kana ? o.kanaType : '漢字') + '</span></div>'+
            '</div>'+
          // end question block
          '</div>';
          ...
          if (++Genki.dataBackupReminderCount >= 10) {
            ...
            setTimeout(function() {
              GenkiModal.open({
                ...
                '<div class="center">'+
                  '<a id="save-exercise-data" class="button" download="Tobira Exercise Score Data.txt" href="data:text/plain;charset=utf-8,' + (storageOK && localStorage.TobiraResults ? encodeURIComponent(localStorage.TobiraResults.replace(/\n/g, '\r\n')) : '') + '"><i class="fa">&#xf019;</i><span class="en">Save Data</span><span class="ja">セーブする</span></a><br><br>'+
      ...
      exerciseList : function () {
        ...
        if (Genki.exercises) {
          var attrs = 'class="lesson-title" onclick="Genki.toggle.list(this);" onkeydown="event.key == \'Enter\' && Genki.toggle.list(this);" tabindex="0"', // lesson-title attrs
              ...
              currentGroup,
              group = '',
              groups = /grammar-wb-\d+|kanji-wb-\d+|kanji-review-wb-\d+/,
              groupTitles = {
                'grammar-wb' : '文法ワークブック',
          ...
          for (; i < j; i++) {
            ...
            if (!new RegExp(lesson).test(linkData[0].replace('study-tools', 'appendix'))) {
              lesson = /^appendix|^study-tools/.test(linkData[0]) ? 'appendix' : 
                       linkData[0].replace(/(lesson-\d+)\/.*/, '$1');

              list += '</ul><h4 ' + attrs + '>' + /*lesson.charAt(0).toUpperCase() + lesson.replace(/-/, ' ').slice(1)*/ (/lesson-\d+/.test(lesson) ? '第' + lesson.replace(/lesson-/, '') + '課' : '巻末とツール') + '</h4><ul id="' + lesson + '">';
              group = '';
            }
      ...
      GenkiModal.open({
        ...
        content :
          '<div class="kanji-stroke-order center">'+
            ...
            '<a class="button" href="' + (kana ? getPaths() + 'resources/images/stroke-order/sasagami-' + kana + '.jpg' : 'https://jisho.org/search/' + kanji + '%20%23kanji') + '" target="_blank" title="' + (GenkiLang == 'ja' ? 'jisho.orgで書き順を見る' : 'View stroke order on jisho.org') + '"><i class="fa">&#xf002;</i><span class="en">View on jisho.org</span><span class="ja">jisho.orgで見る</span></a><br>'+
            '<div class="stroke-order-container"><img src="' + img + '" alt="' + ( GenkiLang == 'ja' ? '書き順' : 'stroke order' ) + '"/></div>'+
          '</div>'
    ...
    init : function () {
      if (Genki.exercises) {
        ...
        if (Genki.active.exercise) {
          ...
          result.insertAdjacentHTML('beforebegin', '<h2 id="exercise-title" class="center" ' + (Genki.active.exercise[2] ? 'data-page="Tobira' + /*(lesson < 13 ? 'I' : 'II') +*/ (/grammar-wb/.test(Genki.active.exercise[0]) ? ' Grammar Power' : /kanji-wb|kanji-review-wb|kanji-vocab/.test(Genki.active.exercise[0]) ? ' Power Up Your Kanji' : '') + ': ' + Genki.active.exercise[2] + '"' : '') + '>' + (
            lesson == 'appendix' ? '巻末' :

...
(function (window, document) {
  ...
  window.TobiraRandomList = storageOK && localStorage.tobiraRandomList ? JSON.parse(localStorage.tobiraRandomList) : [];
  ...
  window.GenkiSettings = {
      ...
      GenkiModal.open({
        ...
        '<ul class="genki-settings-list">'+
          ...
          '<li>'+
            ...
            '</span>'+
            '<a id="settings-save-exercise-data" class="button" download="Tobira Exercise Score Data.txt" href="data:text/plain;charset=utf-8,' + (storageOK && localStorage.TobiraResults ? encodeURIComponent(localStorage.TobiraResults.replace(/\n/g, '\r\n')) : '') + '"><i class="fa">&#xf019;</i><span class="en">Save</span><span class="ja">セーブする</span></a>'+
    ...
    randomExerciseList : function () {
      ...
      var parseExercises = function () {
        ...
        for (var list = '', selected = '', i = 0, j = GenkiExercises.length, data, checked, item; i < j; i++) {
          checked = TobiraRandomList.indexOf(GenkiExercises[i]) != -1;
          ...
          if (!/study-tools|appendix/i.test(data[0])) {
            item = '<div data-lesson="' + data[0].replace(/.*?lesson-(\d+).*/, 'L$1') + '" data-path="' + data[0] + '" data-name="' + data[1] + '">'+
                '<input class="def-selector genki_input_hidden" type="checkbox" onchange="GenkiSettings.updateRandomList(this, \'' + GenkiExercises[i] + '\');"' + (TobiraRandomList.indexOf(GenkiExercises[i]) != -1 ? ' checked' : '') + '>'+
      ...
      if (!window.GenkiExercises) {
        ...
        exercises.src = getPaths() + 'resources/javascript/exercises/tobira.min.js';
    ...
    updateRandomList : function (caller, data) {
      ...
      // remove exercise from the random list
        ...
        TobiraRandomList.splice(TobiraRandomList.indexOf(data), 1);
      ...
      localStorage.tobiraRandomList = JSON.stringify(TobiraRandomList);
    },
